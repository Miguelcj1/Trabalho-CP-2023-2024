...
int main()
{
    ...
    // Aumento do número de átomos da fase 2
    // N = 10*216;
    N = 5000;   
    Vol = N/(rho*NA);
...
/* void computeAccelerations() {
    ...
    #pragma omp parallel for schedule(dynamic) reduction(+:Potential,a[:N][:3]) private(rij, rSqd, rSqd3, rSqd7, f, ai0, ai1, ai2, term1, term2)
...
__global__ void computeAccelerationsKernel(int N_, double* r_, double* a_, double *Pot_){
    int i = blockIdx.x * blockDim.x + threadIdx.x; // índice i
    if(i < N_-1){ // pode haver uma thread cujo id/índice do loop i não esteja correcto. Pelo que essa thread não deve fazer cálculos
        int i3 = 3*i, j3; // usado para aceder ao array 'a' que antes era matriz
        double f, rSqd;
        double rij[3];

        double rSqd3, rSqd7, rijf[3];
        double sigma6, term1, term2, r2;
        sigma6 = 1.0; // a variável sigma é igual a 1.0
        Pot_[i] = 0.0;

        // Equivalente ao primeiro loop da computeAccelerations que inicializa a matriz 'a'
        a_[i3] = 0.0;
        a_[i3+1] = 0.0;
        a_[i3+2] = 0.0;
        // inner loop j da computeAccelerations original
        for (int j = (i + 1) * 3; j < N_ * 3; j += 3){ // temos de iterar assim por causa de r_ e a_ serem arrays a simular uma matriz 
            j3 = 3*j;
            rij[0] = r_[i3]   - r_[j3];
            rij[1] = r_[i3+1] - r_[j3+1];
            rij[2] = r_[i3+2] - r_[j3+2];
...
void computeAccelerationsCUDA(){
    double *d_r, *d_a, *PotGPU; // cópias das matrizes 'r' e 'a' e da variável Pot que vão para a memória da GPU
    double PotCopy[N]; // para guardar os valores do potencial das diversas threads 
    
    cudaMalloc((void**)&d_r, sizeof(double) * N * 3); // aloca memória para a matriz 'r' na GPU
    cudaMalloc((void**)&d_a, sizeof(double) * N * 3); // aloca memória para a matriz 'a' na GPU
    cudaMalloc((void**)&PotGPU, sizeof(double) * N); // aloca memória na GPU para guardar os valores da variável Pot

    cudaMemcpy(d_r, r, sizeof(double) * N * 3, cudaMemcpyHostToDevice); // copia os dados da matriz r do host para a matriz da GPU

    int nBlocks = (N + N_THREADS_PER_BLOCK - 1) / N_THREADS_PER_BLOCK; // calcula o número de blocos necessários tendo em conta o tamanho do N

    computeAccelerationsKernel<<<nBlocks, N_THREADS_PER_BLOCK>>>(N, d_r, d_a, PotGPU);
    cudaMemcpy(a, d_a, sizeof(double) * N * 3, cudaMemcpyDeviceToHost); // copia os resultados da matriz 'a' da GPU para a matriz 'a' do host
    cudaMemcpy(PotCopy, PotGPU, sizeof(double) * N, cudaMemcpyDeviceToHost); // copia os valores do potencial das threads da GPU para o host

    P = 0.0;
    for(int i = 0; i < N-1; i++){ // faz o reduce da Potencial
        P += PotCopy[i];
    }
    P *= 8*epsilon;
    // Libertação da memória alocada na GPU
    cudaFree(d_r);
    cudaFree(d_a);
    cudaFree(PotGPU);
}
